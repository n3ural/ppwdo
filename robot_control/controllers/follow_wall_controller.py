...
class FollowWallController:
    """
    The FollowWallController class implements a control strategy for a robot to follow walls while avoiding obstacles.
    It utilizes sensor data and a Proportional-Integral-Derivative (PID) control mechanism to determine and maintain
    a specific distance from walls. The class calculates heading vectors, updates critical points, and adjusts robot
    orientation in real-time based on wall proximity and desired following direction (left or right).
    """

    def __init__(self, supervisor):
        """
        Initialize the FollowWallController with a reference to the supervisor.

        Sets up sensor placements, desired wall-following distance, control gains, and other parameters
        necessary for wall-following. It also initializes key vectors and data structures for managing
        the robot's heading and wall-following behavior.
    ...
    def update_heading(self):
        """
        Generate and store new heading vectors and critical points for wall-following.

        Based on the robot's proximity to walls and the desired wall-following direction (left or right),
        this method calculates new heading vectors, updates critical points, and adjusts the robot's orientation.
        The method leverages the calculate_fw_heading_vector function to compute the heading vectors for
        both left and right wall-following directions.
        """
    ...
    def execute(self):
        """
        Implement the wall-following control strategy.

        This method determines the wall-following direction (left or right) based on the robot's current state.
        It calculates the time elapsed since the last control iteration, computes error terms for PID control,
        derives the angular and translational velocities for the robot, and sets these velocities for execution.
        The method ensures that the robot follows the wall at an optimal distance and adjusts its speed and
        orientation based on detected wall proximity and desired following direction.
        """
    ...
    def calculate_fw_heading_vector(self, follow_direction):
        """
        Compute a wall-following vector in the robot's reference frame.

        This method determines the desired heading vector for the robot to follow a wall in a specified direction
        (left or right). It takes into account sensor data, wall proximity, and desired following direction to
        generate a directional vector that guides the robot alongside the wall at an optimal distance.

        Parameters
        ----------
        follow_direction : int
            The desired wall-following direction. Can be either FWDIR_LEFT or FWDIR_RIGHT.

        Returns
        -------
        tuple
            The heading vector and its component vectors, as well as the vectors representing the followed surface
